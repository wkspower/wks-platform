...
A local demo proves the concept. A production deployment proves it to your client. WKS is designed for isolated per-client instances: one Postgres database, one MinIO object-store, one URL, one license — no shared infrastructure. Each deployment is a three-container stack you bring up with a single `docker compose` command.
...
The compose file at `docker/docker-compose.prod.yml` is the authoritative per-instance production surface. It brings up three services:
