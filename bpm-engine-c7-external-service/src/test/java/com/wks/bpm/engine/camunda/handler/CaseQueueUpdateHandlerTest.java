...
public class CaseQueueUpdateHandlerTest {

	@InjectMocks
	private CaseQueueUpdateHandler handler;
	...
	public void testExecute() throws Exception {
		...
		String expectedQueue = "queue123";
		CaseInstance expectedCaseInstance = CaseInstance.builder().queueId(expectedQueue).build();

		when(externalTask.getTenantId()).thenReturn("someTenant");
		when(externalTask.getBusinessKey()).thenReturn(expectedBusinessKey);
		when(externalTask.getVariable("queue")).thenReturn(expectedQueue);
	...
	assert resultCaseInstance.getQueueId().equals(expectedQueue);
	}
