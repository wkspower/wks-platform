 ...
 * carries the cap signal without minting a new error code or HTTP header — {@code truncated =
 * true} means "the server returned the oldest {@code items.size()} entries and there are more"
 * (server cap = 500 in Sprint 12; refined by Story 13-4 filters).
 *
 ...
 * @param truncated {@code true} when the server cap was reached (more pending tasks exist than
 *     were returned); always {@code false} when {@code items.size() < cap}.
 */
