 ...
 * direct-edit path) on both the applied and blocked branches so the SI audit log records the intent
 * regardless of result.
 *
 * <p>Per [[feedback_audit_after_commit]]: subscribers MUST listen with
 * {@code @TransactionalEventListener(phase = AFTER_COMMIT)} — never {@code log.info} inside a
 * {@code @Transactional} scope, because Postgres aborts on a subsequent DB exception while the
 * audit line already fired. The {@link com.wkspower.platform.audit.EditAuditEmitter} listener
 ...
 * @param fieldId the field the caller attempted to mutate (one event per blocked field; on APPLIED,
 *     multiple field events may be emitted from a single update)
