 ...
 * ExecutionSignalHandler#onSignal(ExecutionSignal)} via the package-visible {@link
 * #emit(ExecutionSignal)} method invoked by {@link
...
public class BpmnWorkflowAdapter implements WorkflowAdapter {
  ...
  private static final Logger log = LoggerFactory.getLogger(BpmnWorkflowAdapter.class);

  private final AtomicReference<ExecutionSignalHandler> handlerRef = new AtomicReference<>();
  ...
  public BpmnWorkflowAdapter() {
    ...
    //     -> caseStatusListener -> bpmnWorkflowAdapter -> cibSevenWorkflowEngine.
  }
  ...
  public void attach(CaseTypeRef caseType, AttachmentScope scope) {
    ...
    // WorkflowAdapterBinder.register is the single source of truth for routing resolution.
    //
    ...
    log.debug("BpmnWorkflowAdapter attached for caseType={} scope={}", caseType, scope);
  }
  ...
  public void detach(CaseTypeRef caseType) {
    ...
    // case type continue to route normally. Phase-0 has one global ExecutionSignalHandler shared by
    ...
    log.info(
        "BpmnWorkflowAdapter: caseTypeId={} version={} marked as detached — emit() will skip"
  ...
  public ExecutionSignalSubscription onExecutionSignal(ExecutionSignalHandler handler) {
    ...
    // (WorkflowAdapterPortIsolationTest). Idempotent for the SAME handler instance so that the
    // eager boot-time registration (BpmnWorkflowAdapterRegistrar ApplicationReadyEvent) and the
    // WorkflowAdapterBinder.computeIfAbsent path can both succeed without racing.
    handlerRef.compareAndSet(null, handler);
    if (handlerRef.get() != handler) {
      throw new IllegalStateException(
          "BpmnWorkflowAdapter already has a different registered handler — single-subscriber"
  ...
  public String start(CaseInstanceRef caseInstance) {
    ...
    // Phase-0 — WorkflowAdapter.start is exposed but CaseService.create still drives engine start
  ...
  public void cancel(CaseInstanceRef caseInstance) {
    Objects.requireNonNull(caseInstance, "caseInstance");
    log.debug("BpmnWorkflowAdapter cancel (no-op pre-4.4b) for case={}", caseInstance.id());
  }
  ...
  public void emit(ExecutionSignal signal) {
    ...
    if (detachedCaseTypeRefs.contains(emitRef)) {
      log.debug(
          "BpmnWorkflowAdapter.emit: signal dropped for detached caseTypeId={} version={} kind={}"
              + " caseId={}",
          emitRef.caseTypeId(),
          emitRef.version(),
          signal.kind(),
    ...
    ExecutionSignalHandler handler = handlerRef.get();
    if (handler == null) {
      log.warn(
          "BpmnWorkflowAdapter.emit invoked before handler registration — signal dropped: kind={}"
              + " caseId={} source={}",
          signal.kind(),
          signal.caseInstance().id(),
          signal.source());
