...
public final class CriteriaBuilder {
	...
	public Tuple<Criteria, List<Sort>> createCriteriaData() {
		...
		List<Sort> sorted = new ArrayList<Sort>();
		...
		if (cursorSort != null) {
			...
			if ("lt".equals(sortOp)) {
				c2 = where(args.sort()).is(cursorSort)
														.and(args.key())
														.lt(CoreUtils.toObjectId(cursorId));
			} else if ("gt".equals(sortOp)) {
				c2 = where(args.sort()).is(cursorSort)
														.and(args.key())
														.gt(CoreUtils.toObjectId(cursorId));
			}
		...
		return new Tuple<Criteria, List<Sort>>(criteria, sorted);
	}
	...
	public CriteriaDefinition createCriteriaPrevOrNext(Object id, Object sortValue) {
		...
		Criteria c2 = new Criteria();
		if ("lt".equals(sortOp)) {
			c2 = where(args.sort()).is(sortValue)
													.and(args.key())
													.lt(CoreUtils.toObjectId(id));
		} else if ("gt".equals(sortOp)) {
			c2 = where(args.sort()).is(sortValue)
													.and(args.key())
													.gt(CoreUtils.toObjectId(id));
		}
