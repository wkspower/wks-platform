...
@Component
@Slf4j
public class CaseQueueUpdateWorker {
	...
	public void handleJob(final JobClient client, final ActivatedJob job) {
		...
		String queue = gsonBuilder.create().toJson((LinkedHashMap<?, ?>) job.getVariablesAsMap().get("queue"));

		String queuePatch = "{\"queue\": " + "\"" + queue + "\"" + "}";

		caseInstanceApiGateway.patch(businessKey, queuePatch);
	}
