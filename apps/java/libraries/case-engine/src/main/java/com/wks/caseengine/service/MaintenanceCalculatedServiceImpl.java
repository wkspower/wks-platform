...
public class MaintenanceCalculatedServiceImpl implements MaintenanceCalculatedService {
	...
	public AOPMessageVM getMaintenanceCalculated(String plantId, String year) {
		...
		try {
			...
			// List<MaintenanceCalculatedData> maintenanceCalculatedDataList=
			// maintenanceCalculatedDataRepository.findAllByPlantFKIdAndAopYear(UUID.fromString(plantId),year);
			...
			// List<MaintenanceCalculatedDataDTO> maintenanceCalculatedDataDTOList =
			// list.stream()
			// .map(MaintenanceCalculatedDataDTO::new)
			// .collect(Collectors.toList());
			// List<MaintenanceCalculatedDataDTO> maintenanceCalculatedDataDTOList= new
			// ArrayList<>();
			// for(MaintenanceCalculatedData
			// maintenanceCalculatedData:maintenanceCalculatedDataList) {
			// MaintenanceCalculatedDataDTO maintenanceCalculatedDataDTO= new
			// MaintenanceCalculatedDataDTO();
	...
	public List<Object[]> executeDynamicStoredProcedure(String procedureName, String plantId, String siteId,
		...
		try {
			...
			e.printStackTrace();
			throw new RuntimeException("Failed to fetch Error in executeDynamicStoredProcedure", e);
		}
	}
}
