package com.wks.caseengine.utility;

public class ExcelConstants {


    public static final String TITLE = "title";
    public static final String TABLEID = "tableId";
    public static final String HEADERS = "headers";
    public static final String COULMNCOUNT = "columnCount";
    public static final String HEADERSTITLES = "headersTitles";
    public static final String STARTROW = "startRow";
    public static final String STYLES = "styles";
    public static final String AUTOMERGE = "autoMerge";
    public static final String BORDERS = "borders";
    public static final String COLUMNS = "columns";
    public static final String ROWS = "rows";
    public static final String TABLES = "tables";
    public static final String BOLDCOLUMNS ="boldColumns";
    public static final String IS_COLUMN_MERGE_REQUIRED = "isColumnMergeRequired" ;
    public static final String IS_ROW_MERGE_REQUIRED = "isRowMergeRequired";
    public static final String HIDDEN_COLUMNS ="hiddenColumns";
    public static final String STARTING_INDEX_OF_MONTHS ="startingIndexOfMonths";
    public static final String GROUPING ="grouping";
    public static final String DATA_INPUT = "dataInput";
    public static final String TEXT_BEFORE_TITLE = "textBeforeTitle";
    public static final String HIDE_TABLE = "hideTable";


    
}
