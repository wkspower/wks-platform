...
import com.wks.caseengine.repository.AOPMaintenanceDesignRemarksRepository;
...
@Service
public class AOPMaintenanceDesignRemarksServiceImpl implements AOPMaintenanceDesignRemarksService {

	@Autowired
	private AOPMaintenanceDesignRemarksRepository aopMaintenanceDesignRemarksRepository;
	...
	public AOPMessageVM getMaintenanceDesignRemarks(String plantId, String year) {
		...
		try {
			AOPMaintenanceDesignRemarks row = aopMaintenanceDesignRemarksRepository.getData(UUID.fromString(plantId),year);
				if(row!=null) {
	...
	public AOPMessageVM updateMaintenanceDesignRemarks(String plantId, String year,
		...
		try {
			...
			if(aopMaintenanceDesignBasis==null) {
				 aopMaintenanceDesignBasis = new AOPMaintenanceDesignRemarks();
			...
			aopMaintenanceDesignRemarksRepository.save(aopMaintenanceDesignBasis);
			AOPMessageVM aopMessageVM = new AOPMessageVM();
			aopMessageVM.setCode(200);
