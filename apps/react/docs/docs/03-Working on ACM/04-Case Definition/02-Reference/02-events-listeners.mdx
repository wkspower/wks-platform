...
These enum values are kept in lockstep with the Java enums `CaseEventType` and `CaseActionType` and the portal's hardcoded literals. Adding a value on only one side breaks silently, so the [Standard](../../05-configuration-versioning.mdx) pins the vocabulary in [`ext/case-hook-vocabulary.schema.json`](https://github.com/wkspower/wks-platform/blob/develop/packages/case-config-schema/schemas/ext/case-hook-vocabulary.schema.json).
:::
