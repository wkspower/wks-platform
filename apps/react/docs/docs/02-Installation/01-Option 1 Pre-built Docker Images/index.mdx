...
(`ghcr.io/wkspower/*`) — no build and no Maven/Yarn required. The `demo` profile seeds
demo cases, forms, processes, and Keycloak users. Camunda 7 is the engine.

> Pin a specific release with `WKS_VERSION` in `.env` (defaults to the latest release).
> Camunda 8 is experimental and outdated — see `experimental/camunda8/`.
