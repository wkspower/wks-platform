...
<Tabs>
    <TabItem value="custom" label="Custom Domain" default>
    ...
    <TabItem value="local" label="Localhost">
        Since the domain isn't customized and MINIO_HOST is set to the default (minio), you'll need to modify your system's hosts file to ensure file uploads work correctly. Follow these steps:
        1. Edit the `/etc/hosts` file with administrative privileges. You can use Nano or any other text editor. Enter the following command in your terminal:
            ```bash"
            sudo nano /etc/hosts
            ```
        2. Append `minio` to the line that contains `localhost`. This points requests for minio to your local machine, which is useful for development and testing. Update the file to look like this:
            ```bash title="/etc/hosts"
            127.0.0.1 localhost minio
            ```
        Save and exit the editor. For Nano, you can do this by pressing Ctrl+O to write the changes, followed by Ctrl+X to close the editor.
    </TabItem>
</Tabs>