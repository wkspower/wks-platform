...
const STGGrid = ({ hoursRows = [] }) => {
  ...
  const nestedColumns = [
    { field: 'assetName', title: 'Asset Name', width: 150, minWidth: 150, type: 'text', editable: false, locked: true },
    { field: 'utilityDistributed.name', title: 'Utility Distributed', width: 150, minWidth: 150, type: 'text', editable: false, locked: true },
    { field: 'utilityDistributed.sapCode', title: 'Distributed SAP Code', width: 150, minWidth: 150, type: 'text', editable: false, locked: true },
    { field: 'utilityGenerated.name', title: 'Utility Generated', width: 150, minWidth: 150, type: 'text', editable: false, locked: true },
    { field: 'utilityGenerated.sapCode', title: 'Generated SAP Code', width: 150, minWidth: 150, type: 'text', editable: false, locked: true },
    { field: 'assetType', title: 'Asset Type', width: 150, minWidth: 150, type: 'text', editable: false, locked: true, hidden: true },
    {
      title: headerMap[4],
      children: [
    ...
    { field: 'remarks', title: 'Remarks', width: 250, type: 'textarea', editable: true, minWidth: 250 },
  ]
  ...
  const saveChanges = async () => {
    ...
    try {
      await InputApiService.saveOperationHours(keycloak, PLANT_ID, AOP_YEAR, { steamResponse: payload })
      setModifiedCells({})
      setSnackbarOpen(true)
  ...
  const handleExcelUpload = async (file) => {
    ...
    setLoading(true)
    try {
      const response = await InputApiService.saveSteamResponseExcel(file, keycloak, PLANT_ID, AOP_YEAR)
      if (response?.code === 200) {
        ...
        link.href = url
        link.setAttribute('download', 'Error File - STG Shutdown and Operational.xlsx')
  ...
  const handleExport = async () => {
    ...
    try {
      await InputApiService.exportSteamResponseExcel(keycloak, PLANT_ID, AOP_YEAR)
