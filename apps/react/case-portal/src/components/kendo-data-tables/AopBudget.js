...
export default function AopBudget() {
  ...
  // compute a readable oldYear label only if thisYear is in the expected format
  ...
  const handleCalculateMonthwiseAndTurnaround = useCallback(async () => {
    try {
      setLoading(true)
      const storedPlant = localStorage.getItem('selectedPlant')
      const localPlantId = storedPlant ? JSON.parse(storedPlant).id : plantId
      const theYear = localStorage.getItem('year')
  ...
  }, [fetchData, keycloak, plantId])

  const handleCalculate = () => handleCalculateMonthwiseAndTurnaround()
