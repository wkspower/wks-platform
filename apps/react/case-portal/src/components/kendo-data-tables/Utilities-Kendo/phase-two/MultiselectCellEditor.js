...
export const MultiselectCellEditor = ({ dataItem, field, onChange, options = [], textField = 'text', valueField = 'value', placeholder = 'Select...', tagLimit }) => {
  const storedValues = dataItem[field] ?? []
  
  // Find the objects that match the stored values array
  const getInitialSelection = () => {
    if (!Array.isArray(storedValues) || !storedValues.length || !options.length) return []
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      ...
      const valuesToSend = localValue.map(item => {
        if (item && typeof item === 'object' && item[valueField] !== undefined) {
...
export const MultiselectDisplayCell = ({ dataItem, field, options = [], textField = 'text', valueField = 'value', ...tdProps }) => {
