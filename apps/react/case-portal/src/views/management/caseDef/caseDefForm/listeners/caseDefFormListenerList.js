...
export const CaseDefFormEvents = ({ caseDef, setCaseDef }) => {
    ...
    const handleRemoveHook = (hookIndex) => {
        setCaseDef({ ...caseDef, caseHooks: caseDef.caseHooks.filter((element, index) => index !== hookIndex) });
        closeHookForm();
    };
    ...
    return (
        <Grid rowSpacing={4.5} columnSpacing={2.75}>
            <Grid item xs={12} md={7} lg={10}>
                <MainCard sx={{ mt: 2 }} content={false}>
                    {hooks ? (
                        <Box>
                            {isLoading ? (
                                <Box display="flex" alignItems="center" justifyContent="center" height={650}>
                                    <CircularProgress />
                                </Box>
                            ) : hooks.length ? (
                                                ...
                                                <Card>
                                                    <CardContent>
                                                        ...
                                                        <Typography variant="h6" gutterBottom>
                                                            When task <strong>{hook.taskDefKey}</strong> in process <strong>{hook.processDefKey}</strong> is completed
                                                        </Typography>
                                                    </CardContent>
                                                    <Accordion defaultExpanded={true}>
                                                        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                                                            <Box component={AssignmentTurnedIn} color="green" />
                                                        ...
                                                        <AccordionDetails>
                                                            <Typography>
                                                                {formatActionList(hook.actions).split(' AND ').map((action, index) => (
                                                                    <React.Fragment key={index}>
                                                                        {index > 0 && <strong> <br />AND </strong>}
                                                                        {action}
                                                                    </React.Fragment>
                                                                ))}
                                                            </Typography>
                                                    ...
                                                    <CardActions>
                                                        <IconButton color="primary" size="small" onClick={() => handleEditHook(index)}>
                                                            <EditOutlined />
                                                        </IconButton>
                                                        <IconButton color="secondary" size="small" onClick={() => handleRemoveHook(index)}>
                                                            <DeleteOutline />
                                                        </IconButton>
                                                    </CardActions>
                                                </Card>
                                ...
                                <Typography variant="body1">No listeners found for the selected action type.</Typography>
                            )}
                        </Box>
                    ) : (
                        <Typography variant="body1">Select an action type to view associated listeners.</Typography>
                    )}
                </MainCard>
            </Grid>
            ...
            <CaseDefFormEventsForm
                ...
                onSubmit={selectedHookIndex !== null ? handleReplaceHook : handleAddHook}
            ...
            <IconButton color="primary" size="large" onClick={openHookForm} style={addButtonStyle}>
