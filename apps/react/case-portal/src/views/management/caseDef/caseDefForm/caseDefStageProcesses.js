...
export const CaseDefStageProcesses = ({ open, handleClose, stage, updateProcesses }) => {
    const keycloak = useSession();
    const [processes, setProcesses] = React.useState(stage.processesDefinitions || []);
    const [newProcess, setNewProcess] = React.useState({ definitionKey: '', autoStart: false });
            ...
            <DialogContent>
                <List>
                    {processes.map((process, index) => (
                        <ListItem key={index}>
                            <ListItemText primary={`Definition Key: ${process.definitionKey}`} secondary={`Auto Start: ${process.autoStart ? 'Yes' : 'No'}`} />
                            <ListItemSecondaryAction>
                                <IconButton edge="end" onClick={() => handleDeleteProcess(index)} title="Delete Process">
                                    <DeleteIcon />
                                </IconButton>
                            </ListItemSecondaryAction>
                        </ListItem>
                ...
                <div style={{ marginTop: '20px' }}>
                    {processesDefinitions && (
                        <FormControl key="ctrlStagesLCProcess" variant="outlined" sx={{ mt: 3 }} fullWidth>
                            ...
                            <Select
                                ...
                                onChange={(e) => setNewProcess({ ...newProcess, definitionKey: e.target.value })}
                            >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', marginTop: '10px' }}>
                        <Checkbox
                            ...
                            onChange={(e) => setNewProcess({ ...newProcess, autoStart: e.target.checked })}
                            color="primary"
                        />
