...
export function DashboardPage() {
  ...
  return (
    <div className="px-6 py-6 max-w-5xl">
      ...
      <p className="text-foreground-muted text-[13px] mt-0.5">Here's a snapshot of your workspace.</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5">
        ...
        <Stat
          label="License"
          Icon={Shield}
          value={user?.roles.includes('admin') ? '—' : 'OK'}
          loading={false}
          to="/admin/license"
        />
      </div>
      ...
      <div className="mt-8 grid lg:grid-cols-2 gap-4">
        <section className="rounded-lg border border-border bg-canvas p-4">
          ...
          <ul className="space-y-1.5 text-[13px]">
            <li>
              <Link to="/cases" className="text-[var(--primary)] hover:underline">
                Browse cases
              </Link>
            </li>
            <li>
              <Link to="/tasks" className="text-[var(--primary)] hover:underline">
                Work on tasks
              </Link>
            </li>
            {user?.roles.includes('admin') && (
              <li>
                <Link to="/admin" className="text-[var(--primary)] hover:underline">
                  Admin panel
                </Link>
              </li>
            )}
        ...
        <section className="rounded-lg border border-border bg-canvas p-4">
          ...
          <ul className="space-y-1.5 text-[12px] text-foreground-muted">
            <li>
              Press <kbd className="rounded border border-border bg-surface-hover px-1">⌘K</kbd> to jump
              anywhere.
            </li>
...
function Stat({
  ...
  return (
    ...
    >
      ...
      <div className="mt-1.5 text-2xl font-heading font-semibold">{loading ? <Spinner /> : value}</div>
    </Link>
  );
}
